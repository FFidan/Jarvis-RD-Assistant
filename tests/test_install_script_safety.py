...
def test_first_run_smoke_proves_project_ownership_and_cleanup():
    ...
    secret_probe = 'existing_secret_file="$(find "$REPO_ROOT/secrets"'
    assert secret_probe in text
    assert text.index(secret_probe) < text.index(project_probe), (
        "a clean first-run checkout must refuse pre-existing ignored secret files "
        "before acquiring project teardown ownership"
    )
    ...
    assert (
        ...
        and "-name '*.txt' -print -quit" in teardown
    ), "teardown must remove secret files generated by this clean-checkout run"
