"""Unit tests for platform_api.routers.auth — magic-link cooldown.
...
async def test_request_link_cooldown_skips_second_insert() -> None:
    ...
    """
    from platform_api.routers.auth import RequestLinkBody, RequestLinkResponse, request_link
    ...
    with patch("platform_api.routers.auth.send_magic_link", AsyncMock()):
        with patch("platform_api.routers.auth.log_audit", AsyncMock()):
...
async def test_request_link_cooldown_logs_info_on_suppression() -> None:
    ...
    from platform_api.routers.auth import RequestLinkBody, request_link
    ...
    with patch("platform_api.routers.auth.send_magic_link", AsyncMock()):
        with patch("platform_api.routers.auth.log_audit", AsyncMock()):
            with patch(
                "platform_api.routers.auth.logger",
            ) as mock_logger:
...
def test_build_magic_link_warns_on_unset_app_base_url_in_production(monkeypatch, caplog) -> None:
    ...
    from platform_api.routers.auth import _build_magic_link
    ...
    with caplog.at_level(logging.WARNING, logger="platform_api.routers.auth"):
    ...
    warnings = [
        ...
        if r.name == "platform_api.routers.auth" and r.levelno == logging.WARNING
    ]
...
async def test_request_link_unknown_email_sent_true_no_insert() -> None:
    ...
    with patch("platform_api.routers.auth.send_magic_link", AsyncMock()) as send_mock:
        with patch("platform_api.routers.auth.log_audit", AsyncMock()):
            with patch("platform_api.routers.auth.log_event", AsyncMock()) as event_mock:
