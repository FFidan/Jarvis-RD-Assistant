...
def _make_pulse_client(user_id_override=None):
...
async def test_papers_brief_idor_user_id_filter_no_search():
    """GET /api/papers/brief SQL must include IS NOT DISTINCT FROM user_id guard (no search)."""
    import httpx
...
def test_pulse_stats_idor_user_id_filter():
    """GET /api/pulse/stats SQL must include IS NOT DISTINCT FROM for user_id."""
    ...
    assert "IS NOT DISTINCT FROM" in sql, (
        f"pulse/stats SQL must include IS NOT DISTINCT FROM for user_id IDOR guard; got:\n{sql!r}"
    )
...
def test_pulse_debug_idor_user_id_filter():
    ...
    assert "IS NOT DISTINCT FROM" in first_call_sql, (
        f"pulse/debug SQL must include IS NOT DISTINCT FROM for user_id IDOR guard; got:\n{first_call_sql!r}"
    )
