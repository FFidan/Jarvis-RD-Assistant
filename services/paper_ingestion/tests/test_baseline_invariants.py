...
async def test_baseline_pulse_decks_unique_nulls_not_distinct(
    ...
    await conn.execute("INSERT INTO pulse_decks (deck_date, user_id) VALUES ('2026-04-28', NULL)")
    await conn.execute("INSERT INTO pulse_decks (deck_date, user_id) VALUES ('2026-04-28', 43)")
    with pytest.raises(asyncpg.UniqueViolationError):
        await conn.execute(
...
async def test_baseline_paper_user_state_state_check_rejects_invalid(
    ...
    with pytest.raises(asyncpg.exceptions.CheckViolationError):
        await conn.execute(
            "INSERT INTO paper_user_state (paper_id, user_id, state) VALUES ($1, NULL, 'archived')",
            paper_id,
        )
...
async def test_baseline_jsonb_columns_are_jsonb_and_roundtrip_object(
    ...
    for col in ("result", "error"):
        ...
        assert kind == "object", f"job_progress.{col} round-trip must stay 'object', got {kind!r}"
...
@pytest.mark.asyncio(loop_scope="session")
async def test_baseline_thread_user_id_fkey_is_cascade(baseline_conn: asyncpg.Connection) -> None:
    ...
    assert rule == "CASCADE", f"thread_user_id_fkey: expected ON DELETE CASCADE, got {rule!r}"


@pytest.mark.asyncio(loop_scope="session")
async def test_baseline_thread_cascades_on_user_delete(baseline_conn: asyncpg.Connection) -> None:
    ...
    uid = await conn.fetchval(
        "INSERT INTO users (email, role) VALUES ('baseline-083@example.com', 'user') RETURNING id"
    )
    thread_id = await conn.fetchval(
        "INSERT INTO thread (user_id, title, progress) VALUES ($1, 'kept', 0.5) RETURNING id",
        uid,
    )
    kept_count = await conn.fetchval("SELECT count(*) FROM thread WHERE id = $1", thread_id)
    assert kept_count == 1
    await conn.execute("DELETE FROM users WHERE id = $1", uid)
    cascaded_count = await conn.fetchval("SELECT count(*) FROM thread WHERE id = $1", thread_id)
    assert cascaded_count == 0, (
...
@pytest.mark.asyncio(loop_scope="session")
async def test_baseline_project_questions_user_id_fkey_is_cascade(
    baseline_conn: asyncpg.Connection,
) -> None:
    """project_questions_user_id_fkey has delete_rule == CASCADE in the regenerated baseline."""
    conn = baseline_conn
    rule = await _fk_delete_rule(conn, "project_questions", "project_questions_user_id_fkey")
    assert rule == "CASCADE", (
...
@pytest.mark.asyncio(loop_scope="session")
async def test_baseline_project_questions_cascades_on_user_delete(
    baseline_conn: asyncpg.Connection,
) -> None:
    ...
    conn = baseline_conn
    uid = await conn.fetchval(
        "INSERT INTO users (email, role) VALUES ('baseline-084@example.com', 'user') RETURNING id"
    )
    ...
    pq_count = await conn.fetchval("SELECT count(*) FROM project_questions WHERE id = $1", pq_id)
    assert pq_count == 1
    await conn.execute("DELETE FROM users WHERE id = $1", uid)
    assert (
        await conn.fetchval("SELECT count(*) FROM project_questions WHERE id = $1", pq_id) == 0
...
@pytest.mark.asyncio(loop_scope="session")
async def test_audit_log_append_only_rules_present(baseline_conn: asyncpg.Connection) -> None:
...
@pytest.mark.asyncio(loop_scope="session")
async def test_audit_log_delete_is_silent_noop(baseline_conn: asyncpg.Connection) -> None:
...
@pytest.mark.asyncio(loop_scope="session")
async def test_author_alert_log_three_col_unique_present(baseline_conn: asyncpg.Connection) -> None:
    ...
    assert row is not None, (
        "author_alert_log must have a 3-col unique index (tracked_author_id, paper_id, user_id)"
    )
    assert "UNIQUE" in row["indexdef"], f"3-col index must be UNIQUE, got: {row['indexdef']}"


@pytest.mark.asyncio(loop_scope="session")
async def test_author_alert_log_two_col_unique_absent(baseline_conn: asyncpg.Connection) -> None:
...
@pytest.mark.asyncio(loop_scope="session")
async def test_author_alert_log_user_id_fk_set_null(baseline_conn: asyncpg.Connection) -> None:
...
@pytest.mark.asyncio(loop_scope="session")
async def test_baseline_papers_zotero_columns_present(baseline_conn: asyncpg.Connection) -> None:
...
@pytest.mark.asyncio(loop_scope="session")
async def test_tracked_authors_three_col_unique_constraint(
    baseline_conn: asyncpg.Connection,
) -> None:
    """tracked_authors_name_s2_unique must be a 3-col UNIQUE NULLS NOT DISTINCT
    on (user_id, author_name, s2_author_id).

    Verified: db/init.sql tracked_authors_name_s2_unique constraint definition.

    Two users tracking the same author name must NOT conflict; the same user
    tracking the same author name twice must conflict.
    """
    conn = baseline_conn
    # Confirm 3-col constraint exists by introspection
    conkey_count = await conn.fetchval(
        """
    ...
    assert conkey_count == 3, (
        f"tracked_authors_name_s2_unique must cover 3 columns "
        f"(user_id, author_name, s2_author_id), found {conkey_count}"
    )

    # Behavioural: user A and user B may both track "Alice Smith"
    await conn.execute("INSERT INTO users (id, email) VALUES (9901, 'pi01a@example.com')")
    await conn.execute("INSERT INTO users (id, email) VALUES (9902, 'pi01b@example.com')")

    await conn.execute(
        "INSERT INTO tracked_authors (author_name, s2_author_id, source, user_id) "
        "VALUES ('Alice Smith', NULL, 'manual', 9901)"
    )
    # Must NOT raise — different user_id
    await conn.execute(
        "INSERT INTO tracked_authors (author_name, s2_author_id, source, user_id) "
        "VALUES ('Alice Smith', NULL, 'manual', 9902)"
    )

    # Same user, same name, NULL s2_id → conflict (NULLS NOT DISTINCT)
    with pytest.raises(asyncpg.UniqueViolationError):
        await conn.execute(
            "INSERT INTO tracked_authors (author_name, s2_author_id, source, user_id) "
            "VALUES ('Alice Smith', NULL, 'manual', 9901)"
