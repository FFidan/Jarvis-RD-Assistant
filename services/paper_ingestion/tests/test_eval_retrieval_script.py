...
def test_load_eval_set_reads_jsonl_cases(monkeypatch, tmp_path):
    """A fixed JSONL eval set can supplement or replace live verified findings."""
    module = _load_eval_retrieval(monkeypatch)
    eval_path = tmp_path / "scientific-eval.jsonl"
    eval_path.write_text(
        "\n".join(
            [
                json.dumps(
                    {
                        "query": "Hamiltonian conservation law",
                        "expected_paper_ids": [101, 202],
                        "tags": ["math", "physics"],
                    }
                ),
                json.dumps({"query": "spectral convergence", "expected_paper_id": 303}),
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    cases = module.load_eval_set(eval_path)

    assert cases == [
        module.EvalCase(
            query="Hamiltonian conservation law",
            expected_paper_ids=(101, 202),
            source="scientific-eval.jsonl:1",
            tags=("math", "physics"),
        ),
        module.EvalCase(
            query="spectral convergence",
            expected_paper_ids=(303,),
            source="scientific-eval.jsonl:2",
            tags=(),
        ),
    ]


def test_eval_metrics_count_ranked_relevance(monkeypatch):
    """Recall and nDCG account for the whole ranked result list."""
    module = _load_eval_retrieval(monkeypatch)
    results = [
        {"paper_id": 999, "score": 0.99},
        {"paper_id": 202, "score": 0.88},
        {"paper_id": 101, "score": 0.77},
    ]

    assert module.precision_at_1(results, {101, 202}) == 0.0
    assert module.recall_at_k(results, {101, 202}, 3) == 1.0
    assert module.ndcg_at_k(results, {101, 202}, 3) == pytest.approx(0.6934, rel=1e-3)
...
@pytest.mark.asyncio
async def test_main_counts_failed_findings_in_denominator(monkeypatch, capsys):
    """Failed evaluations remain in the denominator instead of inflating quality."""
    module = _load_eval_retrieval(monkeypatch)
    rows = [
    ...
    output = capsys.readouterr().out
    assert "Precision@1:  50.0%  (1/2)" in output
    assert "Recall@3:     50.0%  (1/2)" in output
    assert "nDCG@3:       50.0%  (1/2)" in output
    assert "Total queries: 2" in output
    assert "Failed queries: 1" in output
    pool.close.assert_awaited_once()


@pytest.mark.asyncio
async def test_main_uses_fixed_eval_set_when_configured(monkeypatch, tmp_path, capsys):
    """EVAL_RETRIEVAL_SET uses curated scientific queries without requiring DB summaries."""
    eval_path = tmp_path / "eval.jsonl"
    eval_path.write_text(
        json.dumps({"query": "Hamiltonian structure", "expected_paper_ids": [404]}) + "\n",
        encoding="utf-8",
    )
    module = _load_eval_retrieval(monkeypatch, EVAL_RETRIEVAL_SET=str(eval_path))
    ...
    monkeypatch.setattr(module, "embed_text", AsyncMock(return_value=[0.1]))
    monkeypatch.setattr(
        module,
        "search_qdrant",
        AsyncMock(
            return_value=[{"paper_id": 404, "chunk_index": 1, "score": 0.95, "content": "ok"}]
        ),
    )
    ...
    output = capsys.readouterr().out
    assert "Evaluating source: fixed eval set" in output
...
@pytest.mark.asyncio
async def test_main_fixed_eval_set_does_not_require_database(monkeypatch, tmp_path, capsys):
    """Curated eval sets run without a live database connection."""
    eval_path = tmp_path / "eval.jsonl"
    eval_path.write_text(
        json.dumps({"query": "Hamiltonian structure", "expected_paper_ids": [404]}) + "\n",
        encoding="utf-8",
    ...
    assert "Evaluating source: fixed eval set" in output
    assert "Precision@1:  100.0%  (1/1)" in output
    module.asyncpg.create_pool.assert_not_awaited()
