"""Direct tests for digest LiteLLM helper behavior without respx."""
...
def _row(paper_id: int, topic_name: str) -> dict:
    """Build the minimal digest query row shape."""
    return {
        "id": paper_id,
...
async def test_generate_weekly_digest_uses_master_key_fallback(monkeypatch):
    ...
    result = await generate_weekly_digest(pool, http_client)
...
async def test_generate_weekly_digest_prefers_api_key(monkeypatch):
    ...
    pool = _make_pool([_row(1, "NLP"), _row(2, "NLP")])
    await generate_weekly_digest(pool, http_client)
...
async def test_generate_weekly_digest_honors_explicit_base_url_override(monkeypatch):
...
async def test_generate_weekly_digest_default_argument_still_overrides_env(monkeypatch):
