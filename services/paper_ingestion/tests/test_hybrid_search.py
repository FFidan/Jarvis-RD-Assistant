...
# ---------------------------------------------------------------------------
# RD-DA-003: BM25 user_id scoping — cross-user isolation
# ---------------------------------------------------------------------------


def _make_pool_with_user_filter(
    owner_rows: list[dict],
    other_user_rows: list[dict],
    requesting_user_id: int,
) -> AsyncMock:
    """Pool mock that simulates JOIN user_library scoping.

    When called with user_id==requesting_user_id, returns owner_rows.
    When called with a different user_id (or none), returns other_user_rows.

    This mirrors the real DB: the BM25 SQL now passes user_id as $3,
    so conn.fetch receives (sql, query, candidate_limit, user_id).
    """
    owner_records = [_dict_to_record(r) for r in owner_rows]
    other_records = [_dict_to_record(r) for r in other_user_rows]

    conn = AsyncMock()

    async def _scoped_fetch(sql, *args, **kwargs):
        # args[2] is user_id when the user-scoped branch is taken ($3 param)
        if len(args) >= 3 and args[2] == requesting_user_id:
            return owner_records
        return other_records

    conn.fetch = _scoped_fetch
    conn.fetchrow = AsyncMock(return_value=None)
    ctx = AsyncMock()
    ctx.__aenter__ = AsyncMock(return_value=conn)
    ctx.__aexit__ = AsyncMock(return_value=False)
...
@pytest.mark.asyncio
async def test_hybrid_search_bm25_scoped_to_user():
    """RD-DA-003: BM25 leg returns only papers visible to user_id=1 (cross-user paper excluded)."""
    embedder = _make_embedder()

    # user_id=1 owns paper 10; paper 99 belongs to another user
    user_rows = [
        {
            "id": 10,
            "title": "User 1 paper",
    ...
    pool = _make_pool_with_user_filter(
        owner_rows=user_rows,
        other_user_rows=[],
        requesting_user_id=1,
    )

    with patch.object(embedder, "search_chunks_global", new_callable=AsyncMock, return_value=[]):
        results = await embedder.hybrid_search("test", pool, limit=10, user_id=1)

    ids = [r["id"] for r in results]
    assert 10 in ids, f"User 1's own paper should be returned: {ids}"
    assert 99 not in ids, f"Cross-user paper 99 must not appear: {ids}"


@pytest.mark.asyncio
async def test_hybrid_search_bm25_cross_user_paper_excluded():
    """RD-DA-003: Paper owned by user 2 must NOT appear when user 1 searches."""
    embedder = _make_embedder()

    # Without user scoping, both rows would be returned.
    # With user scoping (user_id=1), only user 1's row should come back.
    user1_rows = [
        {
            "id": 1,
            "title": "User 1 paper unique term",
            "authors": [],
            "url": "https://example.com/1",
            "abstract": None,
            "published_date": None,
    ...
    pool = _make_pool_with_user_filter(
        owner_rows=user1_rows,
        other_user_rows=[],  # scoped query returns nothing extra
        requesting_user_id=1,
    )

    with patch.object(embedder, "search_chunks_global", new_callable=AsyncMock, return_value=[]):
        results = await embedder.hybrid_search("unique term", pool, limit=10, user_id=1)

    ids = [r["id"] for r in results]
    assert ids == [1], f"Only user 1's paper should appear, got: {ids}"


@pytest.mark.asyncio
async def test_hybrid_search_no_user_id_uses_global_pool():
    """Without user_id, BM25 uses the global (unscoped) paper pool.

    This is the single-tenant / public search path.
    """
    embedder = _make_embedder()
    all_rows = [
        {
    ...
    pool = _make_pool(all_rows)

    with patch.object(embedder, "search_chunks_global", new_callable=AsyncMock, return_value=[]):
        results = await embedder.hybrid_search("test", pool, limit=10)

    # All 3 papers returned — no user filter applied
    assert len(results) == 3
