...
from platform_api.routers._auth_shared import build_verify_link, magic_link_on_cooldown
...
class RequestLinkBody(BaseModel):
    """Request a sign-in link for one email address.

    Attributes
    ----------
    email : EmailStr
        Normalized recipient address subject to the shared length limit.
    """
...
class RequestLinkResponse(BaseModel):
    """Report acceptance without revealing whether the account exists.

    Attributes
    ----------
    sent : bool
        Always ``True`` once the enumeration-safe request is accepted.
    """
...
class VerifyBody(BaseModel):
    """Submit a single-use bearer token from a sign-in link.

    Attributes
    ----------
    token : str
        Unhashed token that Platform verifies and consumes atomically.
    """
...
class UserResponse(BaseModel):
    """Describe the user authenticated by a consumed sign-in token.
...
def _audit_pool(request: Request) -> asyncpg.Pool | None:
