...
from platform_api.routers.auth import UserResponse
...
class PasskeyCapability(BaseModel):
    """Describe whether passkeys are usable for the current deployment.

    Attributes
    ----------
    available : bool
        Whether the configured origin permits WebAuthn ceremonies.
    access_mode : str
        Installer-selected access mode used to explain the result.
    """
...
class PasskeyRegistrationResult(BaseModel):
    """Identify a newly registered passkey.

    Attributes
    ----------
    id : str
        Stable credential identifier.
    nickname : str or None, optional
...
class PasskeyInfo(BaseModel):
    """Return display-safe metadata for one registered passkey.
...
def _app_base_origin() -> str | None:
    """Return the configured public origin without a path.
    ...
    -------
    str or None
        Normalized ``scheme://host[:port]`` origin, or ``None`` when unusable.
    """
