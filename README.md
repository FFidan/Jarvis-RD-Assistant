...
|----------|-------------|
| Database passwords | Not a single variable: each service connects as its own least-privilege PostgreSQL role with its own password file under `secrets/`. `scripts/init-secrets.sh` creates them; the [deployment guide](docs/DEPLOYMENT.md#docker-secrets) lists every one. |
...
See [`.env.example`](https://github.com/limitcycle-oss/jarvis-rd-assistant/blob/main/.env.example) for the full annotated list. For production deployments using Docker Secrets (`_FILE` variants), see the [complete secret inventory](docs/DEPLOYMENT.md#docker-secrets); [`secrets/README.md`](https://github.com/limitcycle-oss/jarvis-rd-assistant/blob/main/secrets/README.md) covers file permissions and rotation.
