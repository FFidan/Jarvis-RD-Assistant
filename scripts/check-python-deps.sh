...
if major_minor_patch(fastapi_version) >= (0, 127, 0):
    raise SystemExit(
        f"host FastAPI {fastapi_version} is outside the Docker runtime cap <0.127.0"
    )

print(f"OK host FastAPI {fastapi_version} matches Docker runtime cap <0.127.0")
PY
