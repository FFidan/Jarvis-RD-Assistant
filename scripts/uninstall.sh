...
declare -A TIER_NAME=([1]=stop [2]=app [3]=data [4]=purge)
...
if [ -n "$REPO_OVERRIDE" ]; then
  ...
  RAW="$(cd "$(dirname "$0")/.." 2>/dev/null && pwd -P || true)"; MANAGED_VIA_FLAG=0
fi
[ -n "$RAW" ] || die "No uninstall target given and none could be derived." "Pass --repo <install-dir>."
REPO="$(realpath "$RAW" 2>/dev/null || true)"
...
# -----------------------------------------------------------------------------
HOME_CANON="$(realpath "$HOME" 2>/dev/null || printf '%s' "$HOME")"
case "$REPO" in
...
_stack_is_up() { [ -n "$(docker compose ps -q 2>/dev/null)" ]; }
...
_data_backup_offer() {
  if _stack_is_up; then
    if [ "$SKIP_ORDINARY" -ne 1 ] && _confirm "Capture a backup with scripts/backup.sh before deleting the volumes?"; then
      ( cd "$REPO" && bash scripts/backup.sh ) || warn "Backup did not complete; whether to continue is your call."
    fi
  else
...
_export_backup_key() {
  local dst="$1" src="$REPO/secrets/backup_encrypt_key.txt"
  if [ -f "$src" ]; then
    mkdir -p "$(dirname "$dst")" 2>/dev/null || true
    cp -p "$src" "$dst" || die "Could not copy the backup key to ${dst}." "Choose a writable path and re-run."
  else
...
_run_tier() {
  ...
  if [ "$TIER" -ge 3 ]; then
    _step "compose-down-volumes" docker compose down --remove-orphans --volumes
  else
    _step "compose-down" docker compose down --remove-orphans
  fi
...
if [ "$DRY_RUN" -eq 1 ]; then
  ...
  if [ "$TIER" -ge 4 ]; then mapfile -t TP_CONFIRMED < <(_third_party_image_pins); fi
  _run_tier
  exit 0
fi
