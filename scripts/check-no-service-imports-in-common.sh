...
# review time when the import sits inside a function body. This grep is the
# enforcement: with the violation present, `uv run tach check` still reports
# "All modules validated!", so nothing else in `make check` catches it.
#
...
if grep -rnE "^[[:space:]]*(from|import)[[:space:]]+(platform_api|paper_ingestion|learning_engine|telegram_bot)\b" "$TARGET"; then
