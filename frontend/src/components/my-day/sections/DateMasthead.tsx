...
const QUOTES = [
  '"Read deeply. Think slowly. Note generously."',
  '"The questions you ask shape the answers you find."',
  '"What is one paper you wish you understood better?"',
  '"Today is for finishing what tomorrow remembers."',
];
...
function MiniStat({ value, label, onClick }: MiniStatProps) {
  return (
    ...
    >
      <span className="font-mono text-[18px] leading-none text-strong tabular-nums">
        {value}
      </span>
      <span className="font-mono text-[9px] uppercase tracking-[0.14em] text-meta">
        {label}
      </span>
    </button>
  );
...
export function DateMasthead() {
  ...
  return (
    <header className="border-b border-hair pb-6 grid grid-cols-[1fr_auto] gap-8 items-end">
      <div>
        <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-meta mb-2">
          RESEARCH LOG · ENTRY {entryNum} · {timeStr}
        </p>
        <h1 className="font-serif text-[28px] leading-tight text-strong">{dateStr}</h1>
        <p className="font-serif italic text-[15px] text-zinc-600 dark:text-zinc-400 mt-1">{quote}</p>
      </div>
