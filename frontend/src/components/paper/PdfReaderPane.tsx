...
/** Evidence anchors (EvidenceTab) dispatch this on `window`. It repeats the
 *  name rather than importing it, because this module is lazy-loaded. */
...
export function PdfReaderPane({ paperId }: PdfReaderPaneProps) {
  ...
  useEffect(() => {
    const onGoto = (event: Event) => {
      const { page, quote } = (event as CustomEvent<PdfGotoDetail>).detail;
