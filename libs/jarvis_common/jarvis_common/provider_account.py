...
class ProviderAccountSnapshot:
    ...
    capability : AccountCapability
        Provider-declared account lookup capability.
    data : dict[str, bool or int or float or str or None], optional
        Allowlisted, non-identifying scalar account fields.
...
async def fetch_provider_account(
    provider_id: str,
    *,
    api_key: str | None,
) -> ProviderAccountSnapshot:
    """Return a capability-gated, sanitized provider account snapshot.
    ...
    api_key : str or None
        Decrypted provider credential supplied only at the outbound boundary.

    Returns
    -------
    ProviderAccountSnapshot
        Account capability, allowlisted scalar data, and a stable error code.
