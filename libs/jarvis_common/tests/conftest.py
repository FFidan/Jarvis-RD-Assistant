...
Mirrors the live_pg_dsn fixture pattern from services/paper_ingestion/tests/conftest.py
so that jarvis_common tests can use Docker-backed PostgreSQL when JARVIS_RUN_LIVE_PG=1.
"""
...
def live_pg_dsn() -> str:  # type: ignore[return]
    ...
    Opt-in: set ``JARVIS_RUN_LIVE_PG=1`` and run tests marked ``live_pg``.
    """
