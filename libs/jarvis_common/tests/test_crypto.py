...
def test_reload_fernet_on_sighup_logs_success_on_main_thread(caplog) -> None:
    """reload_fernet_on_sighup logs success when registration succeeds on main thread.

    Grounding: crypto.py:93 — ``logger.info("fernet_sighup_registered")``.
    Operators need to know the SIGHUP handler was armed so they can rotate keys.
    """
    import logging

    with caplog.at_level(logging.INFO, logger="jarvis_common.crypto"):
        reload_fernet_on_sighup()

    assert any("fernet_sighup_registered" in record.message for record in caplog.records), (
        "reload_fernet_on_sighup should log success message on main thread"
    )


def test_reload_fernet_on_sighup_logs_skip_on_non_main_thread(caplog) -> None:
    """reload_fernet_on_sighup logs skip reason when called from non-main thread.

    Grounding: crypto.py:98 — ``logger.info("fernet_sighup_skipped reason=%s", "non_main_thread")``.
    Operators need to know why SIGHUP registration failed (e.g. thread pool service).
    """
    import logging
    ...
    assert errors == [], "reload_fernet_on_sighup should not raise on non-main thread"
    assert any(
        "fernet_sighup_skipped" in record.message and "non_main_thread" in record.message
        for record in caplog.records
    ), "reload_fernet_on_sighup should log skip reason when called from non-main thread"
