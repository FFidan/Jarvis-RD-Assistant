...
2. **`frontend/nginx.conf`** — update `set_real_ip_from 10.137.241.2/32` and `set_real_ip_from 10.137.241.3/32` to the new static IPs assigned to `caddy` and `caddy_local` in the new subnet. If you skip this step all clients collapse to one rate-limit bucket, causing a self-DoS regression where all requests share a single limit.
...
| Re-embedding is too slow through LiteLLM/Ollama | `scripts/reembed.py` defaults to the runtime LiteLLM path, which is safe but HTTP-bound and sequential | Benchmark locally first: `REEMBED_BENCHMARK=true REEMBED_BACKEND=local python -m scripts.reembed`. If output count and dimension are correct, resume with `REEMBED_BACKEND=local python -m scripts.reembed`. |
