...
### 2.6 Discovery & Pulse (Planned — Phase 1)
...
#### 3.1.1 Pulse — Proactive Discovery (PLANNED Phase 1)
...
- New API router `services/paper_ingestion/app/routers/pulse.py` exposes five endpoints: `POST /api/pulse/generate`, `GET /api/pulse/today`, `GET /api/pulse/history`, `POST /api/pulse/rate`, `GET /api/pulse/explain/{card_id}`.
...
**Anti-bloat commitments:** at most 3 new tables + 1 new column, 1 new migration (018), 1 new Python package, 10 new Python source files, 5 new API endpoints, 4 new frontend files, 0 new Docker services, 0 new pages. All Pulse logic is colocated in `pulse/` and `sources/*_source.py`; no scatter across existing modules.
...
### 8.5 Phase 1 — Discovery & Pulse Subsystem (PLANNED)
...
- Existing telegram_bot `research_pulse.py` gutted and rewritten from ~165 lines to ~40 lines as a thin delivery wrapper.
