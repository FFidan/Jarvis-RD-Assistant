...
- **`paper_ingestion` (PDF parsing, reranking) stays CPU-only on AMD and Intel.** The overlays accelerate Ollama only. An AMD/Intel-accelerated ingestion pipeline is not shipped: the PyTorch ROCm wheel is several times larger than the CUDA one, and Docling has an open ROCm-specific crash upstream — both make the tradeoff unfavorable today. Document processing itself needs neither an accelerator nor internet access: the published images carry the document models, so a processor-only installation converts PDFs without downloading anything the first time a document is opened.
