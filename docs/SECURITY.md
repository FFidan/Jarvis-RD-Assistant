...
`.gitignore` rule.  They have since been untracked (`git rm --cached`) and
rotated (new values generated by `scripts/gen-langfuse-keys.sh`).  The old
values are therefore dead.
